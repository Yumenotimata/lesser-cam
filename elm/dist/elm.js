...
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple2(
		{error: $elm$core$Maybe$Nothing, ws: $elm$core$Maybe$Nothing},
...
var $author$project$Main$SetWebSocket = function (a) {
	return {$: 'SetWebSocket', a: a};
};
	...
	return $elm$json$Json$Encode$object(
		_List_fromArray(
			[
				_Utils_Tuple2(
				'uuid',
				$elm$json$Json$Encode$string(packet.uuid)),
...
var $author$project$WebSocket$sendWebSocket = F2(
	function (ws, message) {
		return A2(
			$lobanov$elm_taskport$TaskPort$call,
			{argsEncoder: $author$project$WebSocket$encodeSendWebSocketJson, _function: 'sendWebSocket', valueDecoder: $elm$json$Json$Decode$string},
			...
			case 'OpenWebSocket':
				...
				var openWebSocketTask = A2(
					...
					A2(
						...
						A2(
							$elm$core$Task$map,
							$author$project$Main$SetWebSocket,
							$author$project$WebSocket$open(url))));
				return _Utils_Tuple2(model, openWebSocketTask);
			case 'SetWebSocket':
				var ws = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							ws: $elm$core$Maybe$Just(ws)
						}),
					$elm$core$Platform$Cmd$none);
			case 'SendWebSocket':
				var ws = msg.a;
				var message = msg.b;
				var sendWebSocketTask = A2(
					$elm$core$Task$attempt,
					$elm_community$result_extra$Result$Extra$merge,
					A2(
						$elm$core$Task$mapError,
											...
											$elm$json$Json$Encode$object(
												_List_fromArray(
													[
														_Utils_Tuple2(
														'path',
														$elm$json$Json$Encode$string('test_path'))
													])))
										]))))));
				return _Utils_Tuple2(model, sendWebSocketTask);
			case 'Throw':
				var error = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
...
var $author$project$Main$OpenWebSocket = function (a) {
	return {$: 'OpenWebSocket', a: a};
};
var $author$project$Main$SendWebSocket = F2(
	function (a, b) {
		return {$: 'SendWebSocket', a: a, b: b};
	});
			...
			[
				...
				_List_fromArray(
					[
						...
						$author$project$Main$OpenWebSocket('ws://localhost:8000/ws'))
					]),
				_List_fromArray(
					[
						$elm$html$Html$text('Open WebSocket')
					])),
				A2(
				$elm$core$Maybe$withDefault,
				$elm$html$Html$text('WebSocket is not open'),
				A2(
					$elm$core$Maybe$map,
					function (ws) {
						return A2(
							...
							_List_fromArray(
								[
									$elm$html$Html$Events$onClick(
									A2($author$project$Main$SendWebSocket, ws, 'Hello'))
								]),
							_List_fromArray(
								[
									$elm$html$Html$text('Send WebSocket')
								]));
					},
					model.ws)),
				A2(
