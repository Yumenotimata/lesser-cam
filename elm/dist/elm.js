...
var $author$project$Main$GetCameraH = function (a) {
	return {$: 'GetCameraH', a: a};
};
var $author$project$Main$SetCameraH = function (a) {
	return {$: 'SetCameraH', a: a};
};
var $author$project$Main$Throw = function (a) {
	return {$: 'Throw', a: a};
};
...
var $author$project$Main$unwrapTask = function (task) {
	return A2(
		...
		A2(
			...
			A2($elm$core$Basics$composeR, $lobanov$elm_taskport$TaskPort$errorToString, $author$project$Main$Throw),
			task));
};
var $author$project$Main$init = function (_v0) {
	var initCameraHandler = A2(
		$elm$core$Task$map,
		$author$project$Main$SetCameraH,
		$author$project$CameraHandler$new(_Utils_Tuple0));
	var getCameraListTask = $author$project$Main$unwrapTask(
		A2(
			$elm$core$Task$andThen,
			function (_v1) {
				return $elm$core$Task$succeed(
					$author$project$Main$GetCameraH(
						function (camH) {
							return $author$project$Main$unwrapTask(
								A2(
									$elm$core$Task$map,
									function (result) {
										if (result.$ === 'Ok') {
											var res = result.a;
											return $author$project$Main$Throw('reslut test');
										} else {
											var err = result.a;
											return $author$project$Main$Throw('okoko');
										}
									},
									$author$project$CameraHandler$getCameraList(camH)));
						}));
			},
			initCameraHandler));
	return _Utils_Tuple2(
		{cameraH: $elm$core$Maybe$Nothing, error: $elm$core$Maybe$Nothing, selectableCameras: _List_Nil},
		getCameraListTask);
};
			...
			case 'SetCameraH':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							cameraH: $elm$core$Maybe$Just(cameraH)
						}),
					$elm$core$Platform$Cmd$none);
			case 'GetCameraH':
				var f = msg.a;
				var _v1 = A2($elm$core$Maybe$map, f, model.cameraH);
				if (_v1.$ === 'Just') {
					var cmd = _v1.a;
					return _Utils_Tuple2(
						model,
						$author$project$Main$unwrapTask(
							$elm$core$Task$succeed(
								$author$project$Main$Throw('fff'))));
				} else {
					return _Utils_Tuple2(
						model,
						$author$project$Main$unwrapTask(
							$elm$core$Task$succeed(
								$author$project$Main$Throw('ggg'))));
				}
			case 'SetSelectableCameras':
				var cameras = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
...
var $author$project$Main$view = function (model) {
	var html = function () {
		var _v0 = model.selectableCameras;
		if (!_v0.b) {
			return $elm$html$Html$text('no camera');
		} else {
			return $elm$html$Html$text('camera list');
		}
	}();
	return A2(
		$elm$html$Html$div,
