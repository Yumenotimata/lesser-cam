...
import { CameraId, CameraService, GetLatestCameraFrameResponse, GetLatestVirtualCameraFrameResponse } from "./camera_pb";
...
customElements.define("elm-canvas", class extends HTMLElement {
    ...
    set cameraId(cameraId: CameraId) {
        info("set cameraId" + cameraId);
    ...
    async render() {
        if (!this.rpcClient || !this._cameraId) return;
