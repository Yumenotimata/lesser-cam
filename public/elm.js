// elm-watch hot {"version":"1.2.3","targetName":"My target name","webSocketPort":53150,"webSocketToken":"7593e59a-aad6-401d-a6b3-2c0bafd97d1e"}
"use strict";
(() => {
...
var $author$project$Proto$Camera$Internals_$defaultProto__Camera__CameraId = {id: 0, name: ''};
...
var $author$project$Proto$Camera$Internals_$decodeProto__Camera__CameraId = A2(
	$eriktim$elm_protocol_buffers$Protobuf$Decode$message,
	$author$project$Proto$Camera$Internals_$defaultProto__Camera__CameraId,
	_List_fromArray(
		[
			A3(
...
var $author$project$Proto$Camera$Internals_$decodeProto__Camera__GetCameraListResponse = A2(
	...
	_List_fromArray(
		[
			...
			1,
			$author$project$Proto$Camera$Internals_$decodeProto__Camera__CameraId,
			function ($) {
				return $.cameraList;
			},
			F2(
...
var $author$project$Proto$Camera$Internals_$encodeProto__Camera__CameraId = function (value) {
	return $eriktim$elm_protocol_buffers$Protobuf$Encode$message(
		_List_fromArray(
			[
				_Utils_Tuple2(
				1,
				$eriktim$elm_protocol_buffers$Protobuf$Encode$int32(value.id)),
				_Utils_Tuple2(
				2,
...
var $author$project$Proto$Camera$Internals_$encodeProto__Camera__VirtualCameraConfig = function (value) {
	return $eriktim$elm_protocol_buffers$Protobuf$Encode$message(
		_List_fromArray(
			[
				_Utils_Tuple2(
	...
	return $eriktim$elm_protocol_buffers$Protobuf$Encode$message(
		_List_fromArray(
			[
				...
				A2(
					$elm$core$Basics$composeR,
					$elm$core$Maybe$map($author$project$Proto$Camera$Internals_$encodeProto__Camera__CameraId),
					$elm$core$Maybe$withDefault($eriktim$elm_protocol_buffers$Protobuf$Encode$none))(value.cameraId)),
				_Utils_Tuple2(
				2,
				A2(
					$elm$core$Basics$composeR,
					$elm$core$Maybe$map($author$project$Proto$Camera$Internals_$encodeProto__Camera__VirtualCameraConfig),
					$elm$core$Maybe$withDefault($eriktim$elm_protocol_buffers$Protobuf$Encode$none))(value.config))
			]));
};
		...
		switch (msg.$) {
			case 'Select':
				...
				if ($elm$core$String$isEmpty(value)) {
					...
					var _v1 = A2(
						...
						function (camera) {
							return _Utils_eq(camera.name, value);
						},
						model.cameraList);
					if (!_v1.b) {
						...
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									selectedCamera: $elm$core$Maybe$Just(camera)
								}),
							$elm$core$Platform$Cmd$none);
					}
				}
			case 'GotCameraList':
				...
				if (result.$ === 'Ok') {
					...
					var task = function () {
						var _v3 = $elm$core$List$head(response.cameraList);
						if (_v3.$ === 'Just') {
							var camera = _v3.a;
							return $GlobalWebIndex$cmd_extra$Cmd$Extra$perform(
								$author$project$Main$Select(camera.name));
						} else {
						...
						$anmolitor$elm_grpc$Grpc$toTask(
							A2(
								...
								A2(
									...
									{
										cameraId: $elm$core$Maybe$Just(cameraName),
										config: $elm$core$Maybe$Just(model.virtualCameraConfig)
									})))) : A2(
						$elm$core$Task$attempt,
						$elm$core$Basics$always($author$project$Main$NoOp),
						$anmolitor$elm_grpc$Grpc$toTask(
...
var $author$project$Main$encodeCameraId = function (cameraId) {
	return $elm$json$Json$Encode$object(
		_List_fromArray(
			[
				_Utils_Tuple2(
				'name',
				$elm$json$Json$Encode$string(cameraId.name)),
				_Utils_Tuple2(
				'id',
				$elm$json$Json$Encode$int(cameraId.id))
			]));
};
...
var $author$project$Main$rpcCameraViewer = F3(
	function (rpcUrl, cameraId, isVirtualCamera) {
		return A3(
			$elm$html$Html$node,
			'elm-canvas',
			_List_fromArray(
				[
					...
					$elm$html$Html$Attributes$property,
					'cameraId',
					$author$project$Main$encodeCameraId(cameraId)),
					A2(
					$elm$html$Html$Attributes$property,
					'isVirtualCamera',
							...
							[
								...
								if (_v0.$ === 'Nothing') {
									...
									return A2(
										...
										_List_fromArray(
											[
												A3($author$project$Main$rpcCameraViewer, 'http://localhost:50051', camera, model.isVirtualCamera)
											]));
								}
							}()
							])),
									...
									[
										...
										_List_fromArray(
											[
												...
												$author$project$Main$selectView(
													A2(
														$elm$core$List$map,
														function ($) {
															return $.name;
										...
										_List_fromArray(
											[
												...
												_List_fromArray(
													[
														A5($author$project$Main$sliderFloatView, 0.0, 100.0, 1.0, model.virtualCameraConfig.resolutionRatio, $author$project$Main$OnResolutionChange)
													]))
											])),
										A2(
										$elm$html$Html$div,
